 ...
 * - Modal is now injected via PHP wp_footer hook (not JS)
 * - Uses data-target attributes for finding phone inputs
 * - Event delegation using $(document.body).on() for WooCommerce AJAX compatibility
 * - State management via CSS classes (body.club-anketa-modal-open, #modal.active)
 * 
 ...
 * - Modal is injected via PHP as direct child of <body> to escape restrictive containers
 ...
 * - Uses data-target attributes for reliable phone input detection
...
(function($) {
    ...
    function init() {
        ...
        // In v3.0.0, modal is injected via PHP wp_footer hook
        // injectModalHtml() now acts as a backup/fallback only
        injectModalHtml();
    ...
    function bindEvents() {
        ...
        // Uses data-target attribute first, then falls back to DOM traversal
        ...
        $(document.body).on('click', '.phone-verify-btn', function(e) {
            ...
            // PRIORITY 1: Use data-target attribute from the verify container
            var $verifyContainer = $btn.closest('.phone-verify-container');
            var targetSelector = $verifyContainer.attr('data-target');
            if (targetSelector) {
                debugLog('Using data-target attribute:', targetSelector);
                $input = $(targetSelector);
                if ($input.length > 0) {
                    debugLog('Found input via data-target');
                }
            }
            
            // PRIORITY 2: Look in the phone-verify-group container
            if (!$input || $input.length === 0) {
                var $container = $btn.closest('.phone-verify-group, .phone-group');
                debugLog('Attempting to find input in container...');
                debugLog('Container found:', $container.length > 0);
                if ($container.length > 0) {
                    debugLog('Container HTML:', $container.prop('outerHTML').substring(0, 200) + '...');
                    $input = $container.find('.phone-local, #billing_phone, #reg_billing_phone, #account_phone, #anketa_phone_local, input[type="tel"]').first();
            ...
            debugLog('FINAL: Input found length:', $input ? $input.length : 0);
            if ($input && $input.length > 0) {
                debugLog('FINAL: Input ID:', $input.attr('id'));
                debugLog('FINAL: Input value raw:', $input.val());
            }
            
            var phone = normalizePhone($input ? $input.val() : '');
            ...
            if (!phone || phone.length !== 9) {
                ...
                if ($input && $input.length > 0) $input.focus();
                return;
            }
